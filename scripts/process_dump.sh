...
# 检查目录是否需要更新符号表
# 参数名避免使用 install_dir：主流程中可能 readonly install_dir，bash 无法用 local 遮蔽只读变量
check_directory_needs_update() {
    ...
    local newest_binary=$(find -L "$_install_root" -type f \( -name "*.so" -o -name "src_executor" \) -printf '%T@ %p\n' | sort -n | tail -1 | cut -d' ' -f2-)
...
# 工具链优先使用本仓库 deps（离线包），其次 ENGINEAI_ROBOTICS_THIRD_PARTY，最后 /opt
_script_dir_for_tools="$(cd "$(dirname "$0")" && pwd)"
_root_for_tools="$(cd "$_script_dir_for_tools/.." && pwd)"
if [[ -x "$_root_for_tools/deps/engineai_robotics_third_party/bin/minidump_stackwalk" ]]; then
  TOOLS="$_root_for_tools/deps/engineai_robotics_third_party/bin"
elif [[ -n "${ENGINEAI_ROBOTICS_THIRD_PARTY:-}" && -x "${ENGINEAI_ROBOTICS_THIRD_PARTY}/bin/minidump_stackwalk" ]]; then
  TOOLS="${ENGINEAI_ROBOTICS_THIRD_PARTY}/bin"
else
  TOOLS="/opt/engineai_robotics_third_party/bin"
fi
...
if [ -z "$SYM_ROOT" ]; then
    ...
    readonly sdk_install_root="$root_dir/build/_install"
    dump_syms_bin="$TOOLS/dump_syms"

    if [[ ! -x "$dump_syms_bin" ]] || [[ ! -x "$TOOLS/minidump_stackwalk" ]]; then
        echo "[Processor] WARN: dump_syms or minidump_stackwalk missing under $TOOLS — skip regenerating symbols."
        if [[ -d "$runtime_symbols_dir" ]]; then
            SYM_ROOT_TO_USE="$runtime_symbols_dir"
        else
            SYM_ROOT_TO_USE=""
        fi
    elif check_directory_needs_update "$sdk_install_root" "$runtime_symbols_dir"; then
        echo "[Processor] Binary files updated, regenerating symbols..."

        mkdir -p "$runtime_symbols_dir"

        find -L "$sdk_install_root" -type f -name "*.so" -print0 | while IFS= read -r -d $'\0' so_file; do
        ...
        executor_bin="$sdk_install_root/bin/src_executor"
